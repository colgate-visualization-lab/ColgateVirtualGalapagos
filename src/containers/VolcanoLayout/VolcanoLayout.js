...
class VolcanoLayout extends Component {
  ...
  nextSlide() {
    if (
      ...
      this.state.progressIndex > this.state.slideIndex
    ) {
      this.setState({
        audioPlaying: false,
        hideAudio: false,
  ...
  prevSlide() {
    ...
    if (this.state.slideIndex == 1) {
      setTimeout(
        () =>
          this.setState({ hideAudio: true, hideBack: true, slideAudio: "" }),
        1
      );
    } else if (this.state.slideIndex == 2) {
      setTimeout(() => this.setState({ slideAudio: Exploration01 }), 1);
      ...
      setTimeout(
        () => this.setState({ hideNext: false, slideAudio: terrainMap2 }),
        1
      );
    }
  }
  ...
  usedForWidth = () => {
    this.setState({
      usedForWidth: this.refForWidth.current.clientWidth,
      usedForWidth2: this.refForWidth2.current.clientWidth,
    });
  };
  /* Lifecycle Functions */
  componentDidMount() {
  ...
  render() {
    ...
    return (
      <div className={classes.mainDiv}>
        ...
        <SlideContent
          Completed={Completed}
          optionalSlide={() => this.optionalSlide()}
          slide={slideIndex}
          videoFinished={() => this.audioFinished()}
          widthRef={usedForWidth}
          widthRef2={usedForWidth2}
        />
        <img
          className={hideBack ? classes.invisible : classes.imgBack}
          onClick={() => this.prevSlide()}
          src={Back}
          style={Completed ? {} : buttonStyle}
        />
        <img
          className={hideNext ? classes.invisible : classes.imgNext}
        ...
        <img
          ref={this.refForWidth}
          src={TerrainMap}
          style={{ height: "100%", visibility: "hidden" }}
        />
        <img
          ref={this.refForWidth2}
          src={Ferd}
          style={{ height: "100%", visibility: "hidden" }}
        />
      </div>
