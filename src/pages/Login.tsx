...
function SignUp({ onSend }: { onSend: Function }) {
  ...
  const [botMessages, setBotMessage] = useState<any[]>(["What's your Username/Email?"]);
    ...
    {showInfo && (
      ...
      <div className="fixed right-0 top-3/4 h-1/12 w-6/12">
        ...
        <div className="w-3/4 grid justify-items-stretch">
          <button className="justify-self-end focus:ring-2" onClick={() => startTransition("/sign_up")}>
            <Text text="Don't have an account?" type="body" color="text-dark" size="sm" />
          </button>
        </div>
      </div>
    </div>
